...
bool SHA1::SHA_GO( const char *lpData_Input, char *lpSHACode_Output, bool raw)
{
	...
	if(raw) {
		//big endian
		char* hh = static_cast<char*>(static_cast<void*>(H));
		for (int i = 0; i < 5; i++)
		{
			// 高低位转换
			for(int j = 0; j < 4; j ++)
			{
				lpSHACode_Output[i * 4 + j] = *(hh + i * 4 + 3 - j);
			}
		}
		//little endian
		//memcpy(lpSHACode_Output, H, 20);
	}
	else {
		for (int i = 0; i < 5; i++)
		{
