...
protected:
	...
	public:
		...
		CConnection() noexcept : Socket(INVALID_SOCKET), BufPos(0), Database(nullptr), Status(SESS_UNCONNECTED), Error(false), Time(0)
		{}
	...
	inline void ParseStringMap(CPack& pack, map<string, CAny>& data);
	...
	map<string, CDatabase*> Databases;	/**< 已打开的数据库 */
	...
	shared_timed_mutex* DataMutexes;	/**< 用于数据存取的互斥锁，每个数据库一个锁 */
